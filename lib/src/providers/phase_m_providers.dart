...
final playerAnalyticsDashboardProvider =
    FutureProvider.family<PlayerAnalyticsDashboard, String>((ref, userId) async {
...
final playerGameStatsProvider =
  ...
  final dashboard = ref.watch(playerAnalyticsDashboardProvider(userId));
  return dashboard.whenData((data) => data.gameStats);
});
...
final playerStreakInfoProvider =
  ...
  final dashboard = ref.watch(playerAnalyticsDashboardProvider(userId));
  return dashboard.whenData((data) => data.streakInfo);
});
...
final performanceTrendProvider =
  ...
  final dashboard = ref.watch(playerAnalyticsDashboardProvider(userId));
  return dashboard.whenData((data) => data.performanceTrend);
});

/// Get difficulty breakdown
final difficultyBreakdownProvider =
    FutureProvider.family<List<DifficultyBreakdown>, String>((ref, userId) async {
  final dashboard = ref.watch(playerAnalyticsDashboardProvider(userId));
  return dashboard.whenData((data) => data.difficultyStats);
});
...
final analyticsInvalidationProvider =
    StateNotifierProvider<AnalyticsInvalidationNotifier, List<String>>(
        (ref) {
  final service = ref.watch(analyticsDashboardServiceProvider);
