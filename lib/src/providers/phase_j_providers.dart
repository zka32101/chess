...
final aiLessonGenerationServiceProvider =
    Provider((ref) => AILessonGenerationService.instance);

// ========== Game Analysis Providers ==========

/// Analyze single game with AI evaluation
final gameAnalysisProvider =
    FutureProvider.family<GameAnalysis, String>((ref, gameId) async {
...
final openingRecommendationsProvider =
    FutureProvider<List<AIOpeningRecommendation>>((ref) async {
  final service = ref.watch(aiLessonGenerationServiceProvider);
...
final aiGeneratedLessonsProvider =
    FutureProvider.family<List<AIGeneratedLesson>, (String, int)>(
        (ref, args) async {
  final service = ref.watch(aiLessonGenerationServiceProvider);
  final auth = FirebaseAuth.instance;
  final userId = auth.currentUser?.uid;
...
final endgameInsightsProvider =
    FutureProvider<List<EndgameInsight>>((ref) async {
  final service = ref.watch(aiLessonGenerationServiceProvider);
  final auth = FirebaseAuth.instance;
  final userId = auth.currentUser?.uid;
...
final performanceProgressProvider =
    FutureProvider.family<PerformanceProgressAnalytics, Duration>(
        (ref, period) async {
  final service = ref.watch(aiLessonGenerationServiceProvider);
  final auth = FirebaseAuth.instance;
  final userId = auth.currentUser?.uid;
...
final monthlyPerformanceProvider = FutureProvider<PerformanceProgressAnalytics>(
    (ref) async => ref
        .watch(performanceProgressProvider(const Duration(days: 30)).future));

// ========== State Management Providers ==========

/// Track active game analysis
final activeGameAnalysisProvider = StateProvider<String?>((ref) => null);

/// Track lesson recommendations interaction
final lessonInteractionProvider =
    StateNotifierProvider<LessonInteractionNotifier, LessonInteractionState>(
        (ref) => LessonInteractionNotifier());
...
final strengthMetricsSummaryProvider =
    FutureProvider<StrengthMetricsSummary>((ref) async {
  final profile = await ref.watch(playerProfileProvider.future);
