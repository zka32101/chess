...
class AnalyticsAdvancedService {
  static final AnalyticsAdvancedService _instance = AnalyticsAdvancedService._internal();
  ...
  Future<Map<String, dynamic>> getPlayerAnalytics(String userId) async {
    final doc = await _firestore.collection('player_analytics').doc(userId).get();
    return doc.data() ?? {};
  }

  Future<void> recordMoveTime(String gameId, int moveNumber, int timeSpentMs) async {
    await _firestore.collection('move_times').doc(gameId).collection('moves').doc('$moveNumber').set({
  ...
  Future<Map<String, dynamic>> getMoveTimesAnalysis(String userId) async {
    final snapshot = await _firestore.collection('move_times').where('userId', isEqualTo: userId).get();
